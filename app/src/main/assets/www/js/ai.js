...
export const AI_DOCS = [
  ...
  { doc: 'CONTROL', title: 'Steuerung: Joystick→Policy, Buttons, Makros, Szenarien, Schubsen, Kamera, AUSSEHEN (setAppearance), UI (setUI), ⭐ MOTION-KI (motionKi — Motion-Wiedergabe + Steuer-Mix + Geist-Pause + Clip-Wechsel)' },
...
21. tool="motionKi" — ⭐ MOTION-KI-WIEDERGABE (v2.21.0, MotionBrick-artig): die FERTIG trainierte Motion-Policy ANIMIERT den Roboter (die aktive GLB-Referenz liefert Stil/Phase), der Nutzer steuert BEI BEDARF per Joystick/Buttons. args = {on:<bool>, mix:0…1, paused:<bool>, nextClip:<bool>}. on schaltet ein/aus (schaltet die Referenz automatisch auf FOLGT — der Lehrer hängt am Roboter) — danach MUSS der Nutzer in den Modus POLICY wechseln (oder du erwähnst es). mix = Steuer-Mix: 0 = nur Clip (autonome Wiedergabe), 1 = nur Stick, 0,7 = Stick führt, der Clip gibt das Tempo vor. paused = Geist einfrieren (Roboter hält die Pose) / weiterlaufen lassen. nextClip = zum nächsten Clip für diesen Roboter springen. Nutze das für „spiel meine Motion ab und lass mich lenken", „pausier die Bewegung", „nächster Clip".
...
- MOTION-KI („spiel meine trainierte Motion ab und lass mich mit dem Joystick lenken“, „pausier die Bewegung“, „nächster Clip“, „gib mir die volle Steuerung“) → motionKi: {on:true, mix:0.7} (mix 0 = nur Clip, 1 = nur Stick) — der Nutzer muss danach im POLICY-Modus sein (Glückwunsch-Satz einbauen). GEIST-PAUSE = {paused:true}, weiter = {paused:false}, Clipwechsel = {nextClip:true}. Funktioniert nur mit aktiver GLB-Referenz + trainierter Policy (beim ersten Mal: erst trainieren lassen!).
- ARDY-BRÜCKE („Ich will eigene Bewegungen als Lehrer, habe aber kein CUDA“) → der Nutzer erzeugt die Motion mit dem ARDY-Colab-Notebook (kostenlose Cloud-GPU, im GitHub-Repo: scripts/ardy_colab.ipynb — Text-Prompt rein, G1-QPOS-CSV raus) und importiert sie mit dem Button „.csv (ARDY)“ in der GLB-Leiste (nur G1). Danach: wie ein GLB-Clip behandeln — „Referenz“ antippen, BC vortrainieren, PPO trainieren, MOTION-KI abspielen. Die CSV muss von einem --model g1 stammen (36 Spalten); Fehler beim Import= Zeilen/Spalten prüfen, Meldung zeigt die betroffene Zeile.
...
export function validateToolCall(parsed) {
  ...
  // v2.21.0: MOTION-KI — Motion-Wiedergabe mit Steuer-Mix
  if (tool === 'motionKi') {
    const out = {};
    if (args.on !== undefined) out.on = !!args.on;
    if (Number.isFinite(+args.mix)) out.mix = Math.max(0, Math.min(1, +args.mix));
    if (args.paused !== undefined) out.paused = !!args.paused;
    if (args.nextClip !== undefined) out.nextClip = !!args.nextClip;
    return { tool, args: out };
  }

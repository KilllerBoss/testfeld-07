...
import { makeMotionTask, MOTION_R, sampleArdyDisplay } from './motiontask.js'; // v2.28.11: Playback-Glättung für den ARDY-Geist (Sub-Frame-Sampling wie im HF-Space)
...
import { loadArdyRuntime, clearArdyCache, BASIS_ANIMS, deToEn, ardyCapabilities, refreshArdyImports, ardyImportSummary, ardyProbeNotes } from './ardy.js'; // v2.25.0: ARDY Mini AUF DEM GERÄT (Text→Motion ohne Cloud) · v2.27.0: Dateimanager-Import · v2.28.6: Integritätsprobe
    ...
    if (clip) {
      ...
      if (r3d.ghostGroups && isMotion && rr) {
        ...
        if (clip.srcOverlay) {
          ...
          // bis v2.28.10 saß hier das harte fr = Math.floor(phase · clip.n) —
          // bei 20-Hz-ARDY-Motion auf 60-Hz-Rendering sprang der Geist
          // 20× pro Sekunde zwischen diskreten Posen (Nearest-Frame-Treppen)
          // = das gesehene Zittern. Der ARDY-Space auf HuggingFace glättet
          // nur im Playback (gleitender Float-Cursor, Lerp für Positionen/
          // Winkel, Slerp für Basis-Orientierung) — sampleArdyDisplay tut
          // GENAU das auf derselben Timeline-Skala (t = phase·n, wie der
          // „Perfekt“-Stand v2.28.10): volle ARDY-Motion in Echtzeit, jetzt
          // ohne Treppen. GLB-Clips (Else-Zweig unten) bleiben Nearest-Frame.
          // BEFUND (separat, unverändert): die Physik-Referenz (sampleRef)
          // rechnet t = phase·fps — bei n ≠ fps zeitgestreckt (n/fps) mit
          // Mid-Motion-Sprung; Policy-Semantik → Worklog/Nächste-Iteration.
          if (!S._dispQ || S._dispQ.length !== clip.nu) S._dispQ = new Float32Array(clip.nu);
          if (!S._dispH) S._dispH = [0.8];
          if (!S._dispBQ) S._dispBQ = new Float32Array(4);
          const dbq = sampleArdyDisplay(clip, phase, S._dispQ, S._dispH, S._dispBQ);
          S.sim.setGhostPose(gh, S._dispQ, 0, S._dispH[0], rr[0], rr[1], rr[2], dbq);
        } else {
...
function initArdy() {
  ...
  async function ensureRuntime(mode) {
    ...
    // v2.28.6 INTEGRITÄTSPROBE: die Geräte-Sessions wurden beim Laden gegen
    // die CPU-Referenz geprüft — Ergebnisse (inkl. WASM-Fallback je Graph
    // und abgelehnten falschen Import-Dateien) KLAR ins Log, damit der
    // Nutzer sieht, WORAN es lag, falls es vorher explodierte.
    const notes = ardyProbeNotes();
    const fellBack = notes ? notes.filter((m) => m.indexOf('Integritätsprobe') === 0) : [];
    statusEl.textContent = 'ARDY Mini bereit (' + rt.epName.toUpperCase() + (fellBack.length ? ' · ' + fellBack.length + '× WASM-Fallback' : ' · verifiziert') + ' · ' + rt.fps + ' FPS · ' + rt.jointNames.length + ' Gelenke) — ab jetzt offline nutzbar.';
    log('ARDY Mini geladen (' + rt.epName + (fellBack.length ? ', ' + fellBack.length + ' Graph(en) auf WASM gefallen' : ', Integritätsprobe bestanden') + ') — Modell intsuc/Llama-3-ARDY-Mini-Core40-Browser, läuft ab jetzt auf dem Gerät', fellBack.length ? 'warn' : 'ok');
    if (notes) for (const m of notes) log(m, m.indexOf('Integritätsprobe') === 0 ? 'warn' : 'log');
    return rt;
  }

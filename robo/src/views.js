...
(function (global) {
  ...
  /* ---------------- Konstanten (v2.1: aus envs.js hierher) ----------------
   * Nur was die Views brauchen: Arena-Größe + Arm-Maße für den
   * Platzhalter-Rig (bis das MJCF-Mesh-Rig steht). */
  var ARENA_HALF = 10;        // Arena 20×20 m
  var RAY_LEN = 4.0;          // Duck-Sensorreichweite (nur Alt-Rig)
  var N_RAYS = 9;
  var RAY_SPREAD = 100 * Math.PI / 180;
  var ARM = { L1: 0.55, L2: 0.45, L3: 0.25, HB: 0.5, BALL_R: 0.09, DZ: [0.75, 0.09, -0.75] };

  /* ---------------- Arena ---------------- */
  function buildArena(lib, env, robot) {
    var root = new R.Node();
    var floor = new R.Node(lib.plane, COL.floor);
    floor.scale = [ARENA_HALF * 2, 1, ARENA_HALF * 2];
    root.add(floor);
    var grid = new R.Node(lib.grid, COL.grid, { lit: false });
    root.add(grid);

    // Wände
    var wallDefs = [[0, ARENA_HALF + 0.15], [0, -ARENA_HALF - 0.15]];
    var i;
    for (i = 0; i < 2; i++) {
      var wA = new R.Node(lib.unitBox, COL.wall);
      wA.scale = [ARENA_HALF * 2 + 0.6, 0.5, 0.3];
      wA.pos = [0, 0.25, wallDefs[i][1]];
      root.add(wA);
      var wB = new R.Node(lib.unitBox, COL.wall);
      wB.scale = [0.3, 0.5, ARENA_HALF * 2 + 0.6];
    ...
    for (i = 0; i < 4; i++) {
      ...
      post.pos = [cs[i][0] * (ARENA_HALF + 0.15), 0.45, cs[i][1] * (ARENA_HALF + 0.15)];
      root.add(post);
    }
    ...
    return {
      ...
      update: function (env, dt) {
        ...
        if (this.rayBars.length) {
          for (i = 0; i < N_RAYS; i++) {
            var aAbs = env.heading + (i - 4) / 4 * RAY_SPREAD;
            var len = Math.max(0.05, env.rays[i] * RAY_LEN);

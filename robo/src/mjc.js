/* Testfeld·07 ROBOLAB — ECHTES MuJoCo (WASM) für ARMBOT + HUMANOID.
 *   arm  = WidowX 250 6DOF  (Menagerie trossen_wx250s, 6 Gelenke + Greifer)
 *   hum  = ROBOTIS OP3      (Menagerie robotis_op3, 20 Gelenke)
 * Regelung 50 Hz: 4 Substeps à 5 ms, Wanduhr-Pacer (makePacer, v1.2).
 * Policies: robofield-policy-v1-Genome (Builtin-Champions) laufen direkt
 * über nn.js auf den MJ-Geistern — KEIN ONNX/ORT, KEIN Enten-Zweig,
 * KEIN Werkstatt-Fallback: MJ-Boot scheitert laut oder gar nicht.
 * Assets: fetch() same-origin (appassets https) — kein file://-Bridge-Pfad. */
(function (global) {
  'use strict';
  ...
  /* ---------- Asset-Lader: fetch (same-origin https, kein file://) ----------
   * v2.1: Die Robo-Sim läuft unter https://appassets.androidplatform.net
   * (WebViewAssetLoader) — alle Assets sind dort normales same-origin HTTP.
   * Kein Base64-Umweg, kein Chunking, keine AndroidBridge. Fehlt eine
   * Datei, ist das ein HARTER Fehler (kein stiller Fallback). */
  function mjcUrl(rel) {
    return MJC_DIR + String(rel == null ? '' : rel).replace(/^\/+/, '');
  }
  function readAsset(rel) {
    var p = mjcUrl(rel);
    return fetch(p).then(function (resp) {
      if (!resp.ok) throw new Error('Asset fehlt (' + resp.status + '): ' + p);
  ...
  /* ---------- Zustand ---------- */
  var mjc = null;          // { mujoco } — Modelle je Roboter lazy in robots{}
  ...
  /* ---------- init(): MuJoCo-WASM booten (KEIN ORT, KEIN Duck-Modell) ----------
   * Modelle (arm/hum) lädt loadRobot() lazy je Roboter. Der Boot gilt erst
   * mit WASM-Modul — scheitert er, gibt es KEINEN Ersatzkern, nur den Fehler. */
  ...
  function init(onStage) {
    ...
    var p = loadScript('mujoco.wrapped.js')
      .then(function () {
        if (!global.TF07_loadMujoco) throw new Error('MuJoCo-Glue fehlt');
        onStage('MUJOCO WASM …');
  ...
  TF.mjc = {
    ...
    /* Ball neu auf den Tisch/Spawn legen („neues Ziel" für den Arm) */
    resetBall: function (name) {
      var r = robots[name];
      if (!r) return false;
      (name === 'arm' ? armResetBall : humResetBall)(r);
      return true;
    },

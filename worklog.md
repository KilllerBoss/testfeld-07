...
Task ID: 58
Agent: Super Z (Hauptagent)
Task: Nutzer: „Lade ardy und teste einfaches wie idle. Bei mir ist alles explodiert. Schaue an ob du überhaupt etwas fixet. Bis jetzt sind die Probleme gleichgeblieben" (Screenshot: grünes ARDY-Skelett explodiert, Roboter steht in MANUELL)

Work Log:
- ARDY GELADEN UND GETESTET (wie vom Nutzer gefordert): scripts/ardy/ardy_explode_diag.mjs mit „a person stands still, idle" (Seed 7, 4 s, 80 Frames, echtes fp32-Modell, exakter App-Codepfad) → PERFEKT: Hüfte 0,93 m konstant (0,93–0,93), max Horizontalspeed 0,0 m/s, 0 NaN, 0 reparierte Frames, Knochenfehler 0,000 % in beiden Fenstern.
- BEWEISLAGE: Die App-Pipeline (Modell+Decode+Sanitize+Retarget) ist auf CPU nachweisbar korrekt → die Explosion auf dem Gerät kann NUR geräteseitig entstehen: (a) WebGPU-Kernel liefern falsche Werte auch aus fp32-Graphen, (b) importByName matchte nur per BASISNAME (ardy.js) → ein einmal importierter alter fp16-Decoder (36 MB) beantwortete auch die Anfrage „fp32/decoder.onnx.gz" → der giftige fp16-Decoder überlebte ALLE Updates v2.28.2–v2.28.5 (exakt „Probleme gleichgeblieben").
- FIX 1 (ardy.js): NUMERIK-INTEGRITÄTSPROBE — jede Session rechnet beim Laden einen festen synthetischen Prüfdurchlauf (buildTextEncoderProbeInput/buildDenoiserProbeInput/buildDecoderProbeInput, PortableRandom(PROBE_SEED 911741)); Prüfsummen (Σ|x|, max|x|, Spread) gegen eingebrannte CPU-Referenz ARDY_PROBE_REFERENCE (erzeugt von scripts/ardy/ardy_probe_reference.mjs mit denselben Buildern+Modellfiles); verifySessionIntegrity: Abweichung > PROBE_TOLERANCE 0,35 rel. oder NaN/Inf → Session wird auf WASM neu erstellt und ERNEUT geprüft; klare Logzeilen. Proben-Kosten: 1 Denoiser-Forward ≈ 1/30 einer Generierung.
- FIX 2 (ardy.js): DECODER_FP32_BYTES = 71642198 — Decoder-Datei wird nach dem Laden gegen den fp32-Fingerprint geprüft; eine per Basisname durchgerichtete falsche Import-Datei wird ÜBERSPRUNGEN (fetchModelFile expectedBytes) und aus Cache/HF geladen; _importRejection/ardyImportRejection() liefert klare Meldung.
- FIX 3 (ardy.js deToEn): PRE-Regeln mit Sentinel — „steh still/stehe still/steht still/stehen bleiben/halte still" → „stands still" (vorher „steh stands still" = Kauderwelsch → Out-of-Distribution); EN-Identität („stands/stand/standing still") gegen Replacement-Chaining der generischen \bstill\b-Regel; Subjekt-Ergänzung: fehlt „person/…", wird „a person" vorangestellt („idle" → „a person stands still").
- main.js: ensureRuntime loggt Probe-Ergebnisse („Integritätsprobe bestanden" bzw. je Graph WASM-Fallback) + Import-Ablehnungen; Statuszeile zeigt „verifiziert"/„N× WASM-Fallback".
- TESTS: scripts/ardy_probe_test.mjs NEU 22/22 (Checksummen-Mathematik, Builder-Determinismus + Feed-Formen, ECHTE Proben aller 3 Graphen gegen die Referenz mit Abw. < 1e-6, WASM-Fallback-Pfad mit explodiertem Stub-Decoder, Decoder-Fingerprint gegen echte Datei, deToEn-Fälle, Verdrahtung). Pins erweitert: ardy_mirror 12/12, ardy_retry 35/35, physics_filter 42/42, ghost_ground 30/30, qpos 52/52, motionset 49/49, motionset/ui_v2260/ui_v2270 OR-Ketten +2.28.6/47.
- REGRESSION ALLE GRÜN (15 Suiten): probe 22 · math 17 · sanitize 12 · mirror 12 · retry 35 · runtime 18 · physics_filter 42 · ghost_ground 30 · ghost_drive 17 · qpos 52 · motionset 49 · motion_ctrl · ardy_live · ui_v2260 · ui_v2270.
- BUILD: gradle Daemon (assembleRelease, offline) → app-release.apk 28.137.277 bytes; aapt versionCode 47 / versionName 2.28.6; apksigner SHA-256 1c0422b9… IDENTISCH (CN=Trainrobot OU=Testfeld07); Code-Marker (ARDY_PROBE_REFERENCE, DECODER_FP32_BYTES=71642198, WASM-Fallback) im APK nachgewiesen.
- Release v2.28.6 LIVE (siehe 58-Release).

Stage Summary:
- v2.28.6 / versionCode 47: Auf die Frage „schaue an ob du überhaupt etwas fixet" gibt es jetzt einen MESSBAREN ANTWORTWEG: die App beweist bei JEDEM Modell-Laden auf dem Gerät selbst (Numerikprobe gegen CPU-Referenz), dass ihre Sessions korrekt rechnen — und fällt je Graph auf WASM zurück, wenn der GPU-Treiber falsch rechnet. Der zweite, wahrscheinliche Täter (alter fp16-Import-Decoder, der per Basisname alle Updates überlebt hat) wird vom Datei-Fingerprint abgewiesen.
- Auf CPU ist „idle" perfekt: Hüfte 0,93 m konstant, 0,0 m/s, 0 NaN — das Verhalten auf dem Gerät wird nach dem Update in der Log-Box sichtbar sein (Integritätsprobe).

---
Task ID: 59
Agent: Super Z (Hauptagent)
Task: v2.28.7 — Nutzer: „Jetzt ist es wirklich besser aber der skellet ist falsch. Im Bild hab ich richtigen geschickt wie es sein soll" (Screenshot: weißes cskel27-Skelett aus der ARDY-Browser-Demo, 27 Gelenke, dichte Wirbelsäule, lange Arme mit HandEnd/Thumb1, Füße mit ToeBase)

Work Log:
- DIAGNOSE (bildgestützt): Das Referenz-Bild zeigt das VOLLSTÄNDIGE cskel27 (manifest.skeleton: 27 joint_names, parents-Hierarchie, T-Pose-Ruhelage der Demo). Die App zeichnete das grüne Lehrer-Skelett aus srcPos — und srcPos trug nur die 13 IK-Rollen (GHOST_ROLES): keine Hände/HandEnd/Thumb1, keine Zehen, keine Schultern, Wirbelsäule nur Hips→Spine→Head, Arme hingen direkt an „spine". Das Modell LIEFERT alle 27 posedJoints — sie wurden nur nie in srcPos übernommen. Exakt der Report: „Skelett falsch" = Struktur, nicht Spiegelung (die war v2.28.4 korrekt gefixt).
- FIX 1 (retarget.js): SRC_EDGES (26 Kanten = exakte Eltern-Kind-Hierarchie aus model.json) + SRC_ROLES (27 Rollen in manifest-Reihenfolge, hips zuerst) + SRC_ROLE_ALIASES (14 neue Rollen: spine1-3, neck, shoulders, hands, handends, thumb1, toebases; Mixamo-Präfix zuerst, dann plain — normName-Konvention) + resolveSrcJoints(clip, bones): Kern-13 zuerst aus bones (IK unangetastet), 14 Zusatz-Rollen per Alias, Node-Dedupe (kein Gelenk doppelt). retargetToRobot: srcMap → ghostRoles = SRC_ROLES.filter(...), wanted erweitert, srcPos füllt über srcMap (bones bleibt IK-exklusiv). Log-Zeile „Lehrer-Skelett: 27 Gelenke (volle cskel27-Anatomie)".
- FIX 2 (retarget.js): srcBonePairs(idxByRole) — reine Funktion: Kanten aus SRC_EDGES, fehlende Zwischengelenke werden per AHNEN-WALK überbrückt → ALTE gespeicherte Clips (13 Rollen) bekommen EXAKT die alte 12-Paare-Struktur (spine→head, spine→Arm …), neue Clips den vollen 26-Kanten-Baum. Beweis beider Richtungen in Tests.
- FIX 3 (render3d.js): buildSourceGhost nutzt srcBonePairs(this._srcIdx) statt der hartkodierten PAIRS-Liste — Visualisierung ist jetzt layout-agnostisch (13/27/gemischt).
- FIX 4 (retarget.js): groundSrcPosFrame(..., extra=[]) — ToeBase ist der TATSÄCHLICH tiefste Punkt; ohne ihn blieb die Zehenspitze ~1,1 cm im Boden (Diag-Messung). retargetToRobot + groundSrcPosTrack übergeben jetzt ToeBase-Indizes. Legacy-Signatur unverändert (Tests pinnen ALTES Verhalten: Zehen bleiben ohne extra darunter).
- BEWEIS an der ECHTEN KETTE (neu: scripts/ardy/ardy_skeleton27_diag.mjs — fp32-Modell, ArdyRuntime.generate „a person stands still, idle" Seed 7, ArdyClip, retargetToRobot an echter G1-MuJoCo-wasm-Sim): 27/27 Gelenke in srcPos · 0 NaN · Hüfte 0,942 m · Füße+Zehen exakt geerdet (0,0000 m) · Schulter 0,515 m über Hüfte (dichte Wirbelsäule) · Armlänge 0,58 m · Knochen-Drift zwischen Frames 0,00 % · 27-Rollen → 26-Kanten-Baum · 13-Rollen → exakt die alten 12 Paare · pack/unpack-Roundtrip · fitSrcPosToRobot F=0,827 auf Geist-Basishöhe. BEWEISBILDER: scripts/ardy/ardy_skeleton27_render.py → download/ardy_skeleton27_{frame0,mitte,gefitted}.png (Front/Seite/¾ — Struktur identisch zum Nutzer-Referenz-Screenshot; idle-Arme hängen korrekt am Körper, die T-Pose des Screenshots ist die Demo-Ruhelage).
- TESTS: src_skeleton_test.mjs NEU 65/65 (Baum-Integrität, srcBonePairs legacy/full/Teilmenge/leer, resolveSrcJoints cskel27+Mixamo-GLB+dedupe+links-bleibt-links, groundSrcPosFrame Zehen/Legacy/Sprung-Schutz, pack/unpack 27 Rollen, 14 Verdrahtungs-Pins; 6 Erst-Fails waren ALLE Testfehler: Fuß-über-0-Erwartung, −0,031+0,02-Arithmetik, fehlendes resolveBones im Mixamo-Fall, 2 Redundanzen). Pins auf 2.28.7/48 erweitert: physics_filter, ardy_mirror, ardy_retry, ardy_probe, ghost_ground, qpos, ui_v2260, ui_v2270, motionset_v2230.
- REGRESSIONEN ALLE GRÜN (16 Suiten): src_skeleton 65 · physics_filter 42 · ardy_mirror 12 · ardy_retry 35 · ardy_math 17 · ardy_sanitize 12 · ardy_runtime 18 · ardy_probe 22 · ghost_ground 30 · ghost_drive 17 · qpos 52 · motionset 49 · motion_ctrl · ardy_live · ui_v2260 · ui_v2270.
- BUILD: Daemon-Verfahren → app-release.apk 28.139.953 bytes (08:02); aapt versionCode 48 / versionName 2.28.7 · apksigner SHA-256 1c0422b9251e47ce… IDENTISCH (CN=Trainrobot OU=Testfeld07) · APK-Code-Marker: resolveSrcJoints 2 Dateien, srcBonePairs 3, SRC_ROLES/SRC_EDGES 2, fiLToe, VERSION 2.28.7 ✓.
- RELEASE: siehe 59-Release.
...
Task ID: 60
Agent: Super Z (Hauptagent)
Task: v2.28.8 — Nutzer: „Skellet ist jetzt richtig. Aber g1 ist falsch gemappt an den skellet. Er ist nicht an den skellet, sondern etwas innen. Als wäre es nicht skellet sondern exoskelet" (Screenshot: grünes 27-Gelenk-Skelett als Käfig um den G1 — Wirbelsäule über dem Kopf, Schultern breiter, Arme/Beine außerhalb)

Work Log:
- DIAGNOSE (Screenshot + Messung): Das Skelett-STRUKTUR-Fix (v2.28.7) wirkte, aber srcPos trug weiter MENSCHEN-Proportionen: der uniforme Höhen-Fit (v2.28.5) skaliert nur GLEICHMÄSSIG auf Hüfthöhe — cskel27 (≈1,7 m Mensch, Hüfte→Kopf 0,69 m) um den kompakteren G1 (Hüfte→Kopf ≈ 0,5 m) wirkt als Exoskelett.
- FIX 1 (retarget.js): reproportionSrcPos(srcPos, srcJoints, n, lens) — KNOCHENLÄNGEN-TRANSFER: srcPos wird entlang der SRC_EDGES-Hierarchie neu aufgebaut; RICHTUNG jedes Knochens aus den Lehrer-Daten (Pose/Winkel exakt erhalten), LÄNGE aus lens (am Roboter gemessen). KETTEN-AKKUMULATION: Eltern-Position kommt aus dem ÜBERTRAGENEN Stand (Erst-Version las den Original-Elter → Kanten verfälscht — vom Diag-Check „Unterarm 0,308 statt 0,100" aufgedeckt und gefixt; Test pinnt die Akkumulation jetzt explizit). Idempotent, NaN-sicher, Guards.
- FIX 2 (retarget.js): Roboter-Längen-Messung in retargetToRobot (Nullpose ghost0, Distanzen der Körper-Ursprünge — Segmente starr): Beine = hip/knee/ankle-Körper (G1: OS 0,341 / SB 0,300 / Hüft-Breite 0,121), Wirbelkette = senkrechte Becken→Schulter-Niveau-Distanz (0,292), Schlüsselbein-Kette = laterale Schulter-Distanz, Oberarm = Schulter→Ellbogen (0,193), Unterarm = Ellbogen→Handgelenk (0,101), Zehen/Hände proportional zum Glied-Faktor. STOLPERSTEINE umgangen: waist_yaw_link liegt AUF dem Becken (Distanz 0!) und der Menagerie-G1 hat KEINEN Kopf-Body (w_mark = Bodenmarker, ncam=0) — Schulter = HÖCHSTER Vorfahre des Ellbogens (argmax z, body_parentid[elbow] wäre mittleres Oberarm-Glied).
- FIX 3 (retarget.js): Boden-BEZIEHUNG des Lehrers bleibt erhalten — tMin je Frame VOR dem Transfer messen, nach dem Transfer die GLEICHE min-Fußhöhe wiederherstellen (senken UND heben): Stand-Frames geerdet (0,0000 m gemessen), Sprünge bleiben Sprünge; groundSrcPosFrame hebt nur und hätte das kürzere Bein-Skelett in der Lehrer-Hüfthöhe SCHWEBEN lassen (Diag: 0,23 m schwebend → gefixt).
- FIX 4: srcRig-Marker (motion.srcRig=1) — fitSrcPosToRobot überspringt srcRig-Clips (uniformer Fit nur noch für Legacy-13-Bestände); packMotion/unpackMotion persistieren srcRig.
- BEWEIS an der ECHTEN KETTE (scripts/ardy/ardy_skeleton_fit_diag.mjs + _render.py, fp32-Modell + echte G1-Sim, idle Seed 7): RIG-VERGLEICH Skelett↔G1: OS Δ0,9 cm · SB Δ0,0 · Hüft-Breite Δ0,0 · Oberarm Δ1,3 · Unterarm Δ0,0 · Wirbelkette Δ0,9 cm · Hüfthöhe Δ8,1 cm (Lehrer-Pose leicht gebeugt) · Füße 0,0000 m geerdet · Kopf 0,547 vs G1-Geom-Proxy 0,429 m über Hüfte. Gelenk-Abstände Nullpose (illustriert, mischt Pose-Differenz): VORHER max 0,411 m → NACHHER 0,315 m. BEWEISBILD: download/ardy_skeleton_fit.png (rot=Exoskelett VORHER, grün=NACHHER am G1).
- TESTS: src_skeleton_test auf 90 Checks erweitert (Ketten-Akkumulation, Richtungs-Erhalt, Idempotenz, NaN, leeres lens, srcRig-Skip + Persistenz, shoulderOf-Pin, Boden-Beziehungs-Pin, Version 2.28.8/49); Pins aller Suiten auf 2.28.8/49 erweitert (physics_filter, ardy_mirror, ardy_retry, ardy_probe, ghost_ground, qpos, ui_v2260, ui_v2270, motionset_v2230).
- REGRESSIONEN ALLE GRÜN (16 Suiten): src_skeleton 90 · physics_filter 42 · ardy_mirror 12 · ardy_retry 35 · ardy_math 17 · ardy_sanitize 12 · ardy_runtime 18 · ardy_probe 22 · ghost_ground 30 · ghost_drive 17 · qpos 52 · motionset 49 · motion_ctrl · ardy_live · ui_v2260 · ui_v2270.
- BUILD: Daemon → app-release.apk 28.144.285 bytes (08:54); aapt versionCode 49 / versionName 2.28.8 · apksigner SHA-256 1c0422b9… IDENTISCH · APK-Marker (reproportionSrcPos 2 Dateien, shoulderOf, srcRig 3, VERSION 2.28.8) ✓.
- RELEASE: siehe 60-Release.
...
Task ID: 63
Agent: Super Z (Hauptagent)
Task: v2.28.11 — Nutzer: „Perfekt. Zittert aber. Wie ist es bei ardy mini space auf hugginface? Hat man dort mit Glättung?“

Work Log:
- SPACE-ANALYSE (HF intsuc/ardy-mini, Assets mit Sourcemap heruntergeladen → scripts/ardy_space/): Original-Quellen aus Sourcemap extrahiert (vrm-retarget.ts, viewer.ts, main.ts, motion-data.ts, engine.ts, windows.ts). ANTWORT AUF DIE FRAGE: der Space glättet NICHT im Decoder-Stream (kein EMA/Low-Pass/One-Euro, kein Fenster-Naht-Blending — Fensternähte löst das Modell selbst über History-Tokens: createArWindow speist history-frames als Modell-Input), sondern AUSSCHLIESSLICH beim Abspielen: float frameCursor = elapsed·fps (DEFAULT_MOTION_FPS = 20), sampleJointPosition = linearer Lerp zwischen frame0/frame1, sampleRotationTrack = slerpQuaternions (kürzester Weg) → Sub-Frame-Sampling auf 60/120-Hz-Rendering.
- ROOT-CAUSE (Zittern): main.js Render-Loop sampelte die Geist-Pose NEAREST (fr = Math.floor(phase·clip.n) % n) — ARDY liefert 20 fps, das Rendering läuft 60+ Hz → der Geist sprang 20×/s zwischen diskreten Posen = Nearest-Frame-Treppen = das gesehene Zittern. Beweis an echter G1-Sim (scripts/ardy/ardy_smooth_proof.mjs): max Körpersprung je Render-Frame 7,0 mm (alt) → 2,3 mm (neu), Faktor 3,0 (= 60/20).
- FIX 1 (motiontask.js): neuer Export sampleArdyDisplay(clip, phase, outQ, outH, outBQ) — Gelenk-Lerp + Höhen-Lerp + baseQ-Slerp (kürzester Weg: d<0-Flip, linear ab d>0,9995, Normierung), Wrap i1 = (i0+1) % n (endlos glatt am Loop-Punkt). Skala BEWUSST die Anzeige-Skala t = phase·n (volle Timeline in n/fps s — „Perfekt“-Stand v2.28.10 bleibt, nur ohne Treppen).
- FIX 2 (main.js Render-Loop): srcOverlay-Zweig (ARDY) ruft sampleArdyDisplay mit allokationsfreien Scratch-Puffern (S._dispQ/_dispH/_dispBQ) und übergibt an setGhostPose (off=0); GLB-Else-Zweig UNVERÄNDERT (Nearest-Frame, Original-Mesh-Pfad).
- FIX 3 (motiontask.js ghostAnchor 'folgt'): der Anker-Offset interpoliert jetzt (i0/i1/u, Winkel über wrapAngle-Kürzestweg) statt Math.floor — 20-Hz-Treppen im am Roboter hängenden Geist behoben, gleiche Skala wie refRoot.
- NEUER BEFUND (bewusst UNVERÄNDERT — Policy-Semantik/Tiefe): die Physik-Referenz sampleRef rechnet t = phase·fps (API-Skala „Sekunden“ laut qpos_v2220-Test-Kommentar „Sekunden → Frame 12,5“), aber advance() übergibt Loop-Anteile (0..1) → bei n ≠ fps (ARDY-Standard 5 s → n=80, fps=20) läuft die Referenz ZEITGESTRECKT (Faktor n/fps = 4) und sieht nur Frames 0…19 von 0…79 (0,9 s Material, geloopt in 4,0 s), mit Mid-Motion-Loop-Sprung 0,141 rad. Erklärt historische Reports B/E („random Bewegungen/Sprünge“, „bewegen sich unabhängig“). Lösung gehört in eine EIGENE Iteration (obs/reward/advance/Loop-Offset + Policy-Migration + alle motion_*-Tests sind betroffen — kein heimlicher Umbau im Zitter-Fix).
- SUITE NEU: ardy_smooth_test 28 Checks (sampleArdyDisplay-Mathematik inkl. Slerp-Identität/22,5°-Wert/Norm/Wrap, Jitter-Metrik 60 Hz Faktor 3,0, Konsistenz Anzeige↔sampleRef bei n==fps bitnah 1,5e-8, ghostAnchor-'folgt'-Glättung + Kürzestweg-Wrap, Verdrahtungs-Pins, Version). PIN-REPARATUREN: qpos_v2220-Regex 5[0-1]→5[0-2] (versionCode 52), ardy_ghost_only_git-Vergleich HEAD→Tag v2.28.9 (HEAD wandert mit Releases weiter), ardy_probe onnxruntime-Load robust (3 Kandidaten-Pfade — Environment-Reset verschob node_modules nach /home/z/), model.json.gz aus model.json regeneriert (gitignored), pins_22811.py (27 Edits über 13 Suiten).
- REGRESSIONEN GRÜN (19 Suiten): ardy_smooth 28 · ardy_ghost_only 28 · ardy_side 8 · ardy_mirror 13+ · ardy_retry 35 · ardy_probe 22 · ardy_math 17 · ardy_sanitize 12 · ardy_runtime 18 · ardy_live · physics_filter 42 · qpos 52 · motionset 49 · ghost_ground 30 · src_skeleton 90 · ghost_drive 17 · motion_ctrl · canvas_v2200 84 · ui_v2260 · ui_v2270.
- BEWEISBILD: download/ardy_smooth_proof.png (links: Treppen vs. Lerp/Slerp mit 20-Hz-Frames; rechts: Desync-Befund Anzeige↔Physik-Referenz).

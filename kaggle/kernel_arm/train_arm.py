# ==========================================================================
# Testfeld·07 — SELF-CONTAINED Kaggle-Kernel: ARM
...
# ---- train_arm.py (Hauptteil) ----
# Testfeld·07 — Kaggle-Training: ARMBOT
...
def main():
    ...
    print(f"== TESTFELD·07 Training: ARMBOT, {gens} Generationen, Pop 40, MLP 2×32 tanh ==", flush=True)
    champ, fit, history = train("arm", gens=gens, seed=SEED)
    js = genome_to_policy_json(champ, "arm", gens, fit, "kaggle")
    ...
    p = json.loads(js)
    assert p["arch"] == [15, 32, 32, 5] and len(p["weights"]) == 6
